...
def discover_hashtag(tag, ig_user_id, token, version='v25.0', edge='top_media'):
    """Find public hashtag VIDEO leads for editorial screen-recording, not downloads.

    Graph hashtag media exposes like/comment counts when the owner has not hidden
    them. Play/view counts and duration are not available on this edge, so short
    high-view filters are applied after a local recording is probed.
    """
        ...
        rows.append({
            ...
            'engagement': {
                ...
                'note': ('Hashtag ranking uses likes/comments when present; '
                         'view counts and duration require local recording review'),
            },
            'tag': tag,
        })
