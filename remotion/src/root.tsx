...
const variantFromHash = (hash?: string) => {
  if (!hash || hash.length < 2) return 0;
  const value = Number.parseInt(hash.slice(0, 2), 16);
  return Number.isFinite(value) ? value % 3 : 0;
};
...
const BrandIdentity: React.FC = () => {
  ...
  return <>
    {active && variant === 0 && <div style={{position: 'absolute', top: 86, left: 54,
      width: 660, opacity, translate: `${translateX}px 0`, rotate: '-1.5deg',
      background: '#efe6d3', color: '#17191e', border: '2px solid #17191e',
      boxShadow: '12px 14px 0 #17191ecc', padding: '22px 28px 18px',
      fontFamily: 'Arial, sans-serif'}}>
      <div style={{fontSize: 20, letterSpacing: 4, fontWeight: 800}}>BIOTICA MEDIA // FIELD DOSSIER</div>
      <div style={{fontSize: 54, fontWeight: 950, marginTop: 8}}>SATOSHI SHKRELI</div>
      <div style={{display: 'flex', alignItems: 'center', gap: 16, marginTop: 12}}>
        ...
        <div style={{fontSize: 20, fontWeight: 800}}>MEN'S HEALTH INTELLIGENCE</div>
      </div>
    </div>}

    {active && variant === 1 && <div style={{position: 'absolute', top: 82, left: 54,
      width: 690, opacity, translate: `${translateX}px 0`, background: '#09110fef',
      ...
      <div style={{fontSize: 52, lineHeight: 1.05, fontWeight: 900, marginTop: 10,
        color: '#ffffff'}}>SATOSHI SHKRELI</div>
      <div style={{marginTop: 12, fontSize: 22, letterSpacing: 2}}>MEN'S HEALTH // RECEIPTS REQUIRED</div>
      <div style={{height: 5, marginTop: 16, background: '#72f3b1',
        width: `${interpolate(frame, [start, end - 8], [12, 100], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp'})}%`}} />
    </div>}

    {active && variant === 2 && <div style={{position: 'absolute', top: 70, left: 70,
      width: 600, opacity, translate: `${translateX}px 0`, background: '#faf7ef', color: '#101318',
      ...
      <div style={{fontSize: 19, fontWeight: 900, letterSpacing: 5}}>BIOTICA MEDIA</div>
      <div style={{fontSize: 50, fontWeight: 950, marginTop: 6}}>SATOSHI SHKRELI</div>
      <div style={{fontSize: 22, marginTop: 9, fontWeight: 800}}>MEN'S HEALTH // RECEIPTS REQUIRED</div>
      <div style={{display: 'flex', gap: 5, marginTop: 15, height: 30}}>
